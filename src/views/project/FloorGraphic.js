 ...
 */
export default  class FloorGraphic extends mars3d.graphic.BasePointEntity {
  /**
   * 对象添加到图层前创建一些对象的钩子方法，
   * 只会调用一次
   * @return {void}  无
    ...
    for (let i = 0; i < floorCount; i++) {
      ...
      const model = new mars3d.graphic.ModelEntity({
        ...
        attr: {
          origAlt: alt, // 记录原始高度
          layer:i,
          louhao:this.style.louhao,
          address:this.style.address,
  ...
  showFloor(floorNum, time = 1) {
    ...
    for (let i = floorNum; i < this._models.length; i++) {
      const model = this._models[i]
      model.position = new mars3d.LatLngPoint( point.lat,point.lng, maxHeight)
      model.show = false
    }
